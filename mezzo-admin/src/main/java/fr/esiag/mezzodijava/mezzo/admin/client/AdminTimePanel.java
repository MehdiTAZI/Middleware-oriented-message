...
public class AdminTimePanel {
	...
	public Widget onInitialize() {
		...
		Button startButton = new Button("Demarrer CosTime", new ClickHandler() {
			public void onClick(ClickEvent arg0) {
				  // (3) Make the call
				  defaultShellExectuer.execute(new String[]{"/opt/mezzo/costimeserver","start"}, defaultCallback);
			}});

		startButton.ensureDebugId("cwBasicButton-start");

		Button stopButton = new Button("Arreter CosTime", new ClickHandler() {
			public void onClick(ClickEvent arg0) {
				 // (3) Make the call
				ShellExectuer.execute(new String[]{"bash","/opt/mezzo/costimeserver","stop"}, callback);
			}
		});
		...
		// Add a tab
		Button statButton = new Button("Etat CosTime", new ClickHandler() {
			public void onClick(ClickEvent arg0) {
				  // (2) Create an asynchronous callback to handle the result.
				  
				ShellExectuer.execute(new String[]{"bash","/opt/mezzo/costimeserver","status"}, callback);
